...
def rm_handler(arg):#doesnt work 
    print arg.file

def rn_handler(arg):#doesnt work as of now
...
def usage():
    print ' -------------------------------------------------------------------------'
    print ' IS mini-project project, 2015'
    print ' Akankshya kar'
    print ' '
...
def handle_args(args, parser):#This function handle the arguments and leads them to correct function defn
    if args.mode == "get":
        try:
...
def shell():
    ...
    build_parser(parser)
    while True:
        inp = raw_input("Shell for EncryptFS : ")
        try: 
